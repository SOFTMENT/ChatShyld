...
var refresh_handler_exports = {};
__export(refresh_handler_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(refresh_handler_exports);
...
// ../backend/src/shared/token.jwt.ts
var signToken = (payload) => import_jsonwebtoken.default.sign(payload, cfg.jwtSecret, {
  algorithm: "HS256",
...
// ../backend/src/refresh_token/refresh.repo.ts
var import_lib_dynamodb3 = require("@aws-sdk/lib-dynamodb");
var put = (row) => dynamoDoc.send(
...
var revoke = (tokenId) => dynamoDoc.send(
...
// ../backend/src/auth/refresh.service.ts
var import_node_crypto = __toESM(require("node:crypto"));
var b64url = (b) => b.toString("base64url");
var hmac = (secret) => import_node_crypto.default.createHmac("sha256", cfg.refreshPepper).update(secret).digest("base64url");
var issueRefresh = async (userId, meta) => {
  const tokenId = import_node_crypto.default.randomUUID();
  const secret = b64url(import_node_crypto.default.randomBytes(32));
  const token = `${tokenId}.${secret}`;
  const ttl = Math.floor(Date.now() / 1e3) + cfg.refreshTtlDays * 86400;
  await put({
    tokenId,
    userId,
